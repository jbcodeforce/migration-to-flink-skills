...
class TestSparkScripts(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # SQL sources live next to the tests/ package, not under it
        cls.sources_dir = cls.current_dir.parent / "sources"
    ...
    def _create_raw_events(self):
        ...
        event_metadata_type = StructType([
            StructField("device_info", device_info_type, True),
            StructField("location", location_type, True),
        ])
        ...
        raw_events_data = [
            (
                ...
                base_time,
                "click",
                (["premium", "mobile"], {"campaign_id": "camp_001", "source": "organic"}),
                (("iOS", "Safari"), ("US", "New York")),
        ...
        raw_events_schema = StructType([
            ...
            StructField("event_timestamp", TimestampType(), True),
            StructField("event_type", StringType(), True),
            StructField("event_properties", event_properties_type, True),
            StructField("event_metadata", event_metadata_type, True),
            # Column name matches src_event_processing.sql (GET_JSON_OBJECT(raw_event_data, ...))
            StructField("raw_event_data", StringType(), True),
        ])
