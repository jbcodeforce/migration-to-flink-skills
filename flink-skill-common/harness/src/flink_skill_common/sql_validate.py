...
class SqlValidationError(RuntimeError):
    """One or more Flink SQL statements failed validation."""
...
def _validate_parseable(sql: str, kind: SqlKind, index: int) -> list[SqlValidationIssue]:
    """
    Validate if the Flink SQL is parseable usinf sqlglot extension for Flink
    If not, return a list of SqlValidationIssue.
    If the SQL is parseable, return an empty list.
    """
...
def _validate_one(sql: str, kind: SqlKind, index: int) -> list[SqlValidationIssue]:
    ...
    issues: list[SqlValidationIssue] = []
    if kind == "ddl":
        if not is_create_table_statement(stripped):
            return [
                SqlValidationIssue(
                    statement_index=index,
                    kind=kind,
