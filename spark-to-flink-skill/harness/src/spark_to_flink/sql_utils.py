...
_CREATE_OBJECT_NAME_PATTERN = re.compile(
    r"CREATE(?:\s+OR\s+REPLACE)?\s+(?:TEMPORARY\s+VIEW|TABLE)\s+"
...
def extract_spark_object_name(statement: str) -> str | None:
    """Return the table/view name from a CREATE TABLE or TEMPORARY VIEW statement."""
