...
def test_extract_spark_object_name():
    assert extract_spark_object_name("CREATE TABLE foo (id INT);") == "foo"
    assert (
        extract_spark_object_name(
            "CREATE OR REPLACE TEMPORARY VIEW raw AS SELECT 1;"
        )
    ...
    assert extract_spark_object_name("SELECT 1;") is None
    assert extract_spark_object_name("") is None
